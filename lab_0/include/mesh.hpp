...
public:
    ...
    /// @brief Returns a cube centered at the origin
    /// @details Returns a cube centered at the origin (if -x is to the left, +x is to the right, -y is down, +y is up, -z is into the screen, +z is out of the screen, the cube visible)
    static Mesh centeredCube() {
        return Mesh(std::vector<Triangle>({
            // front
            Triangle(Vec(1, 1, 1), Vec(1, -1, 1), Vec(-1, -1, 1)),
            Triangle(Vec(-1, -1, 1), Vec(-1, 1, 1), Vec(1, 1, 1)),
            // back
            Triangle(Vec(1, 1, -1), Vec(-1, -1, -1), Vec(1, -1, -1)),
            Triangle(Vec(-1, -1, -1), Vec(1, 1, -1), Vec(-1, 1, -1)),
