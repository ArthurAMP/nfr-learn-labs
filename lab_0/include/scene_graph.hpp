...
public:
    ...
    std::shared_ptr<TransformNode> addChild(Mesh mesh, Vec pos, Quaternion rotation, Vec scale, std::shared_ptr<TransformNode> parent)
    {
        std::shared_ptr<Mesh> meshPtr = std::make_shared<Mesh>(mesh);
        RenderInfo renderInfo = RenderInfo(meshPtr);
        std::shared_ptr<TransformNode> transformNode = std::make_shared<TransformNode>(Transform(pos, rotation, scale), renderInfo);
