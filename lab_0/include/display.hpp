...
public:
    ...
    void prepare() {
        if (startedStream)
        {
            ...
            // fwrite(rewindStr, sizeof(char), sizeof(rewindStr), stdout);
            std::cout << rewindStr;
            // flush
            fflush(stdout);
        }
        ...
        {
            // clear the terminal
            // fwrite(cleanupStr, sizeof(char), sizeof(cleanupStr), stdout);
            std::cout << cleanupStr;
            fflush(stdout);
            // hide the cursor
    ...
    void draw(const Texture& tex) {
        ...
        // fwrite(this->_outputBuffer, sizeof(char), this->getBufferSize(), stdout);
        std::cout << this->_outputBuffer;
...
private:
    ...
    void hideCursor(bool hide) {
        if (hide) {
            // hide the cursor
            // fwrite("\x1b[?25l", sizeof(char), 6, stdout);
            std::cout << "\x1b[?25l";
        }
        else {
            // show the cursor
            // fwrite("\x1b[?25h", sizeof(char), 6, stdout);
            std::cout << "\x1b[?25h";
        }
    }
};
